...
$out = Join-Path $root '.analysis/pipe-exit-tests'
...
$source = Join-Path $PSScriptRoot 'test-pipe-exit.cpp'
$exe = Join-Path $out 'test-pipe-exit.exe'
$obj = Join-Path $out 'test-pipe-exit.obj'
...
cmd /c $command
if ($LASTEXITCODE -ne 0) { throw 'Pipe exit test compile failed' }
Copy-Item (Join-Path $OpenXrSdk 'native/Win32/release/bin/openxr_loader.dll') $out -Force
& $exe $out
if ($LASTEXITCODE -ne 0) { throw 'Pipe exit tests failed' }
