...
$out = Join-Path $root '.analysis/vr-menu-tests'
...
$source = Join-Path $PSScriptRoot 'test-vr-menu.cpp'
$exe = Join-Path $out 'test-vr-menu.exe'
$obj = Join-Path $out 'test-vr-menu.obj'
...
cmd /c $command
if ($LASTEXITCODE -ne 0) { throw 'VR menu test compile failed' }
Copy-Item (Join-Path $OpenXrSdk 'native/Win32/release/bin/openxr_loader.dll') $out -Force
& $exe $out
if ($LASTEXITCODE -ne 0) { throw 'VR menu tests failed' }
